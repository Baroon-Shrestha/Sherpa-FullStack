...
const createBooking = async (req, res) => {
  try {
    ...
    const {
      ...
      checkIn,
      checkOut,
      numberOfRooms = 1, // default to 1 if not provided
    } = req.body;
    ...
    // Optional: Check if enough rooms are available
    ...
    // Upload ID verification images to Cloudinary
    const idVerificationImages = await uploadImages(
      req.files.idVerificationImages
    );

    // Normalize uploaded images to array
